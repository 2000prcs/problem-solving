...
var exclusiveTime = function(n, logs) {
  ...
  var time;
  var i = 1;
  stack.push(parseInt(logs[0].split(':')[0]));
  var prev = parseInt(logs[0].split(':')[2]);
  while(i < logs.length){
      id = parseInt(logs[i].split(':')[0]);
      status = logs[i].split(':')[1];
      time = parseInt(logs[i].split(':')[2]);
      if(status === 'start'){
          if(stack.length !== 0){
              let peek = stack[stack.length - 1];
              result[peek] = result[peek] || 0;
              result[peek] += time - prev;
          }
          stack.push(id);
          prev = time;
      } else {
...
"1:end:5",
"0:end:6"];
let result = exclusiveTime(2, logs);
console.log(result);
